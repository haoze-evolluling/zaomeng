...
fun PluginsScreen(
    ...
    val zipLauncher = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        ...
        scope.launch {
            runCatching {
                withContext(Dispatchers.IO) { context.contentResolver.readPluginZip(uri) }
            }.onSuccess { (filename, bytes) ->
                viewModel.inspectPackage(filename, bytes)
    ...
    val directoryLauncher = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocumentTree()) { uri ->
        ...
        scope.launch {
            runCatching {
                withContext(Dispatchers.IO) { context.contentResolver.packPluginDirectory(uri) }
            }.onSuccess { (filename, bytes) ->
                viewModel.inspectPackage(filename, bytes)
            }.onFailure(viewModel::reportPackageReadError)
        }
    ...
    if (sourceChooserOpen) {
        AlertDialog(
            ...
            title = { Text("安装插件") },
            text = { Text("选择一个 ZIP 插件包，或选择包含 plugin.json 的插件目录。安装前会先检查清单并显示权限。") },
            confirmButton = {
                Button(onClick = {
